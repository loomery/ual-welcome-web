...
const primaryClass =
  'inline-flex min-h-11 items-center gap-2xs border-2 border-solid border-[var(--color-yellow)] bg-[var(--color-yellow)] p-s font-main text-step-0 font-ual-bold leading-ual-condensed text-ual-dark no-underline transition-colors after:content-["_→"] cursor-pointer ' +
...
export function NextStepCard({ title, body, primary, secondary }) {
  return (
    <article className="space-y-s border-2 border-ual-dark bg-ual-dark p-m text-ual-light dark:border-ual-light dark:bg-ual-light dark:text-ual-dark">
      ...
      <div className="mt-xs flex flex-wrap items-center gap-s">
        ...
        {secondary && (
          <Link
            href={secondary.href}
            className="inline-flex items-center border border-ual-dark-50 px-s py-2xs text-step-d1 font-ual-bold text-ual-light no-underline after:ml-2xs after:content-['_→'] hover:border-(--color-yellow) hover:text-(--color-yellow) focus-visible:border-(--color-yellow) focus-visible:text-(--color-yellow) dark:text-ual-dark"
          >
            {secondary.label}
          </Link>
        )}
